<template>
  <div class="text-card" v-if="shouldShowCard">
    <h1>{{ currentTitle }}</h1>
...
// Dynamically check if the current step has any text to display
const shouldShowCard = computed(() => {
  const currentBlock = content[props.stepIndex - 1];
  return !!(currentBlock && (currentBlock.title || currentBlock.paragraph1 || currentBlock.paragraph2));
});

const currentTitle = computed(() => content[props.stepIndex - 1]?.title || "");
const currentParagraph1 = computed(() => content[props.stepIndex - 1]?.paragraph1 || "");
const currentParagraph2 = computed(() => content[props.stepIndex - 1]?.paragraph2 || "");
</script>
...
@keyframes colorLoop {
  0% {
    --color: hsl(358, 100%, 49%);
  } /* Magenta */
  2.5% {
    --color: hsl(170, 100%, 60%);
  } /* Purple */
  5% {
    --color: hsl(25, 100%, 62%);
  } /* Indigo */
  ...
  10% {
    --color: hsl(169, 100%, 61%);
  } /* Violet */
  12.5% {
    --color: hsl(137, 100%, 53%);
  } /* Lavender */
  15% {
    --color: hsl(348, 97%, 42%);
  } /* Magenta */
  17.5% {
    --color: hsl(335, 100%, 64%);
  } /* Plum */
  20% {
    --color: hsl(19, 100%, 68%);
  } /* Purple */
  ...
  47.5% {
    --color: hsl(196, 100%, 73%);
  } /* Plum */
  50% {
    --color: hsl(186, 98%, 49%);
  } /* Purple */
  52.5% {
    --color: hsl(284, 100%, 58%);
  } /* Magenta-pink */
  55% {
    --color: hsl(76, 100%, 74%);
  } /* Plum */
